...
def print_circle(img,dest,old_circle,number):
    ...
    circles = cv.HoughCircles(img_r,cv.HOUGH_GRADIENT,1,dist,param1=100,param2=10,minRadius=mRad,maxRadius=MRad)
    print(circles)
    min=1000
    index=-1
...
def print_circle_green(img,dest,old_circle,number):
    ...
    if(np.size(circles)!=0):
        if(count==0):
            old_circle=circles[0,initial,:]
        for i in range (int(np.size(circles)/3)):
            if(min>circles[0,i,0] and ((circles[0,i,0]-old_circle[0])**2+(circles[0,i,1]-old_circle[1])**2)<=20**2):
                min=circles[0,i,0]
                print((circles[0,index,0]-old_circle[0])**2+(circles[0,index,1]-old_circle[1])**2)
                index=i
                #print('correct')
...
if __name__ == '__main__':
    ...
    while (success):
        ...
        old_circle_g,G_processed=print_circle_green(img_g,image,old_circle_g,1)
